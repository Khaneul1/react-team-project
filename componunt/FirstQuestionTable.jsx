...
const FirstQuestionTable = ({ isRandom }) => {
    ...
    const handleClick = (e, index, column) => {
        ...
        if (firstClickedValue === null && clickedTagName === 'SPAN') {
            ...
            if (firstClickedValue !== null && clickedTagName !== 'SPAN' || targetElement.style.backgroudColor === '#FFFFFA') {
    ...
    const checkCorrect = () => {
        const isCorrect = JSON.stringify(VitaminInfo) === JSON.stringify(updatedData);
        ...
        return(
            <div style={styles.wrapper}>
                ...
                <table style={styles.table}>
                    ...
                    <tr key={index}>
                        <td><img src={item.food} style={{ width:'150px' }} /></td>
                        <td onClick={(e) => handleClick(e, index, 'nutrient')}>
                        {item.nutrient === '' ? '' : item.nutrient}
                        </td>
                        <td style={{ whiteSpace: 'pre-line'}} onClick={(e) => handleClick(e, index, 'effect')}>
                        {item.effect === '' ? '' : item.effect}
                        </td>
                        <td style={{ whiteSpace: 'pre-line'}} onClick={(e) => handleClick(e, index, 'recommend')}>
            ...
            <div style={styles.wrapper}>
                <table style={styles.table}>
                    ...
                    <tbody>
                        {updatedData.map((item, index) => (
                            <tr key={index}>
                                <td><img src={item.food} alt={`food-${index}`} style={styles.img} /></td>
                                <td>{item.nutrient}</td>
                ...
                <div style={{ display: 'inline' }}>
                    <img src={Remember} style={{ height: '30px', float: 'left', marginRight: '400px' }} />
                </div>
            </div>
        )
    }
