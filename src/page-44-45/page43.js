...
const Page43 = (props) => {
    ...
    const checkCorrectness = (section) => {
        ...
        const selectedSet = new Set(
            Object.keys(selectedCells).filter((key) => selectedCells[key] && key.startsWith(section))
        );
        const isCorrect =
            correctSet.size === selectedSet.size && [...correctSet].every((item) => selectedSet.has(item));
    ...
    return (
        <div style={styles.wrapper} alt="dv">
            <img src={Back2} alt="background" style={{ width: '800px', height: 'auto' }} />
            ...
            <div style={styles.question} alt="subject">
                <h4>
                    아래는 블록을 쌓은 사진입니다.
                    <br />
                    사진과 {'<보기>'}를 참고하여 앞과 옆에서 본 평면도를 클릭하여 완성하세요.
                </h4>
            </div>
            <table style={styles.table} alt="tb">
                <tr alt="tr">
                    ...
                    <td style={styles.tableCell} alt="td">
                        <div style={styles.tableHeader} alt="text">{'<보기>'} <br />위에서 본 평면도</div>
                        <img src={image2} alt="example2" style={styles.imageFixed} />
                    </td>
                </tr>
                <tr alt="tr">
                    <td style={styles.tableCell}>
                        <div style={styles.tableHeader} alt="text">앞에서 본 평면도</div>
                        {[0, 1].map((row) => (
                        ...
                        <button style={styles.button} alt="button1" onClick={() => checkCorrectness('front')}>
                            확인
                        </button>
                    </td>
                    <td style={styles.tableCell}>
                        <div style={styles.tableHeader} alt="text">옆에서 본 평면도</div>
                        {[0, 1].map((row) => (
                            <div key={row} style={{ display: 'flex', justifyContent: 'center' }}>
                                {[0, 1].map((col) => {
                        ...
                        <button style={styles.button} alt="button2" onClick={() => checkCorrectness('side')}>
                            확인
                        </button>
                    </td>
                </tr>
